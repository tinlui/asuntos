...
export class ConsultarComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    for (let index = 0; index < this.consulta.length; index++) {
      this.optionC.push(this.consulta[index]['consecutivo']);
    }
    ...
    for (let index = 0; index < this.consulta.length; index++) {
      this.optionD.push(this.consulta[index]['dirigido']);
    }
    ...
    for (let index = 0; index < this.consulta.length; index++) {
      this.optionS.push(this.consulta[index]['solicita']);
    }
  }
  ...
  private _filterA(value: any): string[] {
    ...
    return this.optionD.filter(
      (option) => option.indexOf(filterValue.dirigido) ===0
    );
  }
  ...
  busqueda(valores: any) {
    if (valores.consecutivo) {
     this.filtrarConsulta.emit ( this.consulta.filter(
    ...
    if (valores.asunto) {
      this.filtrarConsulta.emit ( this.consulta.filter(
        (asunto) => console.log( asunto.asunto.includes(valores.asunto) )
      ));
    ...
    if (valores.dirigido) {
      this.filtrarConsulta.emit ( this.consulta.filter(
        (asunto) => asunto.dirigido.includes(valores.dirigido) 
      ));
    }
